...
export default function SellerEditProductPage() {
  ...
  useEffect(() => {
    document.title = "Urun Duzenle | enolsun.com Satici Merkezi"
    loadData()
  }, [])
  ...
  async function loadData() {
    ...
    if (!storeRes.data) { setLoading(false); return }
    ...
    const { data: product } = await _sb
      .from("products")
      .select("*, product_images(id, url, is_primary, sort_order), product_variants(id, name, value)")
      .eq("id", productId)
    ...
    if (primaryImg) {
      ...
      setExistingImageId(primaryImg.id || null)
    }
    ...
    if (product.product_variants && product.product_variants.length > 0) {
      ...
      setVariants(Object.entries(grouped).map(([name, vals]) => ({ name, values: vals.join(", ") })))
    }
  ...
  async function handleSubmit(asDraft: boolean) {
    ...
    if (error) {
      showToast("Urun guncellenemedi: " + error.message, "error")
      setSubmitting(false)
      return
    }
    ...
    showToast(asDraft ? "Urun taslak olarak guncellendi." : "Urun basariyla guncellendi!", "success")
    ...
    <>
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-neutral-800">Urun Duzenle</h1>
        </div>
      </div>
