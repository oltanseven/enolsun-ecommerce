...
export default function SellerSettingsPage() {
  ...
  const [storeName, setStoreName] = useState("Yeşil Yaprak Atölye")
  const [storeDesc, setStoreDesc] = useState("El yapımı, doğal ve sürdürülebilir ev dekorasyon ürünleri.")
  const [storePhone, setStorePhone] = useState("0532 123 45 67")
  const [storeEmail, setStoreEmail] = useState("info@yesilyaprak.com")
  const [storeAddress, setStoreAddress] = useState("Kadıköy, İstanbul")
  ...
  useEffect(() => { document.title = "Mağaza Ayarları | enolsun.com Satıcı Merkezi" }, [])

  const tabs = [
    { key: "store", label: "Mağaza Bilgileri", icon: <path d="M13.5 21v-7.5a.75.75 0 01.75-.75h3a.75.75 0 01.75.75V21m-4.5 0H2.36m11.14 0H18m0 0h3.64m-1.39 0V9.349m-16.5 11.65V9.35m0 0a3.001 3.001 0 003.75-.615A2.993 2.993 0 009.75 9.75c.896 0 1.7-.393 2.25-1.016a2.993 2.993 0 002.25 1.016c.896 0 1.7-.393 2.25-1.016a3.001 3.001 0 003.75.614m-16.5 0a3.004 3.004 0 01-.621-4.72L4.318 3.44A1.5 1.5 0 015.378 3h13.243a1.5 1.5 0 011.06.44l1.19 1.189a3 3 0 01-.621 4.72m-13.5 8.65h3.75a.75.75 0 00.75-.75V13.5a.75.75 0 00-.75-.75H6.75a.75.75 0 00-.75.75v3.15c0 .415.336.75.75.75z"/> },
    { key: "shipping", label: "Kargo Ayarları", icon: <path d="M8.25 18.75a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0H6.375c-.621 0-1.125.504-1.125 1.125M20.25 18.75V10.5a1.875 1.875 0 00-.254-.955l-2.658-4.587A1.875 1.875 0 0015.72 3.75H12v14.25m0 0h8.25m-8.25 0H5.25"/> },
    ...
    { key: "security", label: "Güvenlik", icon: <path d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z"/> },
  ]
  ...
  return (
    <>
      ...
      {activeTab === "store" && (
        <div className="space-y-6">
          <div className="bg-white rounded-2xl p-6 shadow-align-xs border border-neutral-100">
            <h2 className="text-base font-semibold text-neutral-900 mb-4">Mağaza Bilgileri</h2>
            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="block text-sm font-medium text-neutral-700">Mağaza Adı</label>
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button className="px-4 py-2 border border-neutral-200 rounded-xl text-sm font-medium text-neutral-700 hover:bg-neutral-50 transition-colors">Logo Değiştir</button>
                </div>
              </div>
            </div>
          </div>
      ...
      {activeTab === "shipping" && (
        <div className="space-y-6">
          <div className="bg-white rounded-2xl p-6 shadow-align-xs border border-neutral-100">
            <h2 className="text-base font-semibold text-neutral-900 mb-4">Kargo Ayarları</h2>
            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="block text-sm font-medium text-neutral-700">Ücretsiz Kargo Limiti (&#8378;)</label>
                ...
                <p className="text-xs text-neutral-400">Bu tutarın üzerindeki siparişlerde kargo ücretsiz olur</p>
              </div>
              <div className="space-y-1.5">
                <label className="block text-sm font-medium text-neutral-700">Varsayılan Kargo Firması</label>
              ...
              <div className="space-y-1.5">
                <label className="block text-sm font-medium text-neutral-700">Hazırlama Süresi</label>
                <select value={processTime} onChange={(e) => setProcessTime(e.target.value)} className="block w-full rounded-xl border border-neutral-200 bg-white py-3 px-4 min-h-[48px] text-sm text-neutral-800 shadow-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary-300 focus:border-primary-400 hover:border-neutral-300 appearance-none">
                  <option value="0-1">Aynı gün / 1 iş günü</option>
                  <option value="1-2">1-2 iş günü</option>
                  <option value="2-3">2-3 iş günü</option>
                  <option value="3-5">3-5 iş günü</option>
                </select>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl p-6 shadow-align-xs border border-neutral-100">
            ...
            <div className="space-y-5">
              {[
                { label: "Yeni sipariş bildirimi", desc: "Yeni bir sipariş geldiğinde bildirim al", checked: notifNewOrder, onChange: setNotifNewOrder },
                { label: "Mesaj bildirimi", desc: "Yeni mesaj geldiğinde bildirim al", checked: notifMessage, onChange: setNotifMessage },
                { label: "Değerlendirme bildirimi", desc: "Yeni bir değerlendirme yapıldığında", checked: notifReview, onChange: setNotifReview },
                { label: "Stok uyarısı", desc: "Stok azaldığında uyarı al", checked: notifStock, onChange: setNotifStock },
              ].map((n, i) => (
          ...
          <div className="bg-white rounded-2xl p-6 shadow-align-xs border border-neutral-100">
            <h2 className="text-base font-semibold text-neutral-900 mb-4">Bildirim Kanalları</h2>
            <div className="space-y-4">
      ...
      {activeTab === "security" && (
        <div className="space-y-6">
          <div className="bg-white rounded-2xl p-6 shadow-align-xs border border-neutral-100">
            <h2 className="text-base font-semibold text-neutral-900 mb-4">Şifre Değiştir</h2>
            <div className="space-y-4 max-w-md">
              <div className="space-y-1.5">
                <label className="block text-sm font-medium text-neutral-700">Mevcut Şifre</label>
                <input type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} placeholder="Mevcut şifreniz" className="block w-full rounded-xl border border-neutral-200 bg-white py-3 px-4 min-h-[48px] text-sm text-neutral-800 placeholder:text-neutral-300 shadow-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary-300 focus:border-primary-400 hover:border-neutral-300" />
              </div>
              <div className="space-y-1.5">
                <label className="block text-sm font-medium text-neutral-700">Yeni Şifre</label>
                <input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="Yeni şifreniz" className="block w-full rounded-xl border border-neutral-200 bg-white py-3 px-4 min-h-[48px] text-sm text-neutral-800 placeholder:text-neutral-300 shadow-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary-300 focus:border-primary-400 hover:border-neutral-300" />
              </div>
              <div className="space-y-1.5">
                <label className="block text-sm font-medium text-neutral-700">Yeni Şifre (Tekrar)</label>
                <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Yeni şifrenizi tekrarlayın" className="block w-full rounded-xl border border-neutral-200 bg-white py-3 px-4 min-h-[48px] text-sm text-neutral-800 placeholder:text-neutral-300 shadow-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary-300 focus:border-primary-400 hover:border-neutral-300" />
              </div>
              <button className="px-6 py-2.5 bg-primary-500 text-white rounded-xl text-sm font-semibold hover:bg-primary-600 shadow-md shadow-primary-500/20 transition-all">Şifreyi Güncelle</button>
            </div>
          </div>
          <div className="bg-white rounded-2xl p-6 shadow-align-xs border border-neutral-100">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-base font-semibold text-neutral-900">İki Faktörlü Doğrulama</h2>
                <p className="text-sm text-neutral-500 mt-0.5">Hesabınızı ek bir güvenlik katmanı ile koruyun</p>
              </div>
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-neutral-25">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-neutral-500">İstanbul, TR - Şu anda aktif</p>
                  </div>
                </div>
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-neutral-25">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-neutral-500">İstanbul, TR - 2 saat önce</p>
                  </div>
                </div>
                <button className="text-xs text-error-base font-medium hover:text-error-dark transition-colors">Sonlandır</button>
              </div>
            </div>
          </div>
        </div>
