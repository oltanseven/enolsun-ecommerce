...
export default function Navbar() {
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
              ...
              {NAV_ITEMS.map((item) => (
                item.hasMegaMenu ? (
                  ...
                  >
                    ...
                    >
                      {item.label}
                      <svg className={`w-3.5 h-3.5 transition-transform ${megaOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5"/></svg>
                    </Link>
                  </div>
                ) : (
                  <Link
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              <Link href="/wishlist" aria-label="Favorilerim" className="hidden sm:flex p-2 min-w-[44px] min-h-[44px] items-center justify-center text-neutral-400 hover:text-neutral-600 hover:bg-neutral-50 rounded-lg transition-colors cursor-pointer">
              ...
              <Link href="/cart" className="relative p-2 min-w-[44px] min-h-[44px] flex items-center justify-center text-neutral-400 hover:text-neutral-600 hover:bg-neutral-50 rounded-lg transition-colors badge-pulse cursor-pointer" aria-label="Sepet">
                ...
                {cartCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 w-4.5 h-4.5 min-w-[18px] bg-error-base rounded-full flex items-center justify-center text-[10px] font-bold text-white">{cartCount}</span>
                )}
              </Link>
      ...
      {/* Mega Menu Panel — fixed below navbar */}
      {megaOpen && (
        <div
        ...
        >
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-2">
            <div className="bg-white rounded-2xl border border-neutral-100 shadow-align-xl p-6 animate-fade-in" style={{ animationDuration: '0.15s' }}>
              <div className="flex gap-6">
                {/* Left: category list */}
                <div className="w-56 shrink-0 space-y-0.5 border-r border-neutral-100 pr-6">
                  ...
                  {MEGA_MENU_CATEGORIES.map((cat, idx) => (
                    <Link
                      ...
                      className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors ${activeCat === idx ? 'bg-primary-50' : 'hover:bg-neutral-50'}`}
                    >
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center transition-colors ${activeCat === idx ? 'bg-primary-100' : 'bg-neutral-50'}`}>
                        <MegaCategoryIcon icon={cat.icon} />
                      </div>
                      <span className={`text-sm font-medium transition-colors ${activeCat === idx ? 'text-primary-700 font-semibold' : 'text-neutral-700'}`}>{cat.label}</span>
                    </Link>
                  ))}
                </div>

                {/* Right: subcategories or promo */}
                <div className="flex-1 min-h-[280px]">
                  {activeCat !== null && MEGA_MENU_CATEGORIES[activeCat]?.subcategories ? (
                    <div className="grid grid-cols-3 gap-x-8 gap-y-4 p-2">
                      {MEGA_MENU_CATEGORIES[activeCat].subcategories.map((sub) => (
                        <div key={sub.group}>
                          <p className="text-xs font-semibold text-primary-500 mb-2">{sub.group}</p>
                          {sub.items.map((subItem) => (
                            <Link
                              key={subItem}
                              href={`/products?search=${encodeURIComponent(subItem)}`}
                              onClick={() => setMegaOpen(false)}
                              className="block text-sm text-neutral-500 hover:text-primary-600 hover:pl-1 py-1.5 transition-all"
                            >
                              {subItem}
                            </Link>
                          ))}
                    ...
                    <div className="h-full bg-primary-25 rounded-xl p-6 flex flex-col items-center justify-center text-center">
                      ...
                      <p className="text-sm font-semibold text-neutral-700 mb-1">Tüm Ürünleri Keşfedin</p>
                      <p className="text-xs text-neutral-400 mb-4">Kategori üzerine gelerek alt kategorileri görün</p>
                      <Link href="/products" onClick={() => setMegaOpen(false)} className="inline-flex items-center gap-1.5 px-4 py-2 bg-primary-500 text-white text-xs font-semibold rounded-lg hover:bg-primary-600 transition-colors">
                        Ürünlere Git
